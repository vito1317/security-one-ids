            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($user && preg_match('/^[a-zA-Z0-9._-]+$/', $user) && $user !== 'root' && $user !== 'daemon' && $user !== 'nobody' && $user !== '_mbsetupuser') {
                    ...
                    try {
                        ...
                        if ($returnCode1 !== 0 && stripos($outputStr, 'Permission denied') !== false) {
                            $skipFallbacks = true;
                        }
                    } catch (\Exception $e) {
                        $returnCode1 = 1;
                    ...
                    if ($returnCode1 !== 0 && !$skipFallbacks) {
                        ...
                        try {
                            ...
                            $outputStr2 = trim($process2->getOutput() . ' ' . $process2->getErrorOutput());
                            if ($returnCode2 !== 0 && stripos($outputStr2, 'Permission denied') !== false) {
                                $skipFallbacks = true;
                            }
                        } catch (\Exception $e) {
                            $returnCode2 = 1;
                    ...
                    if ($returnCode2 !== -1 && $returnCode2 !== 0 && !$skipFallbacks) {
                        // Method 3: Set an impossible password hash
                        try {
                            $process3 = new SymfonyProcess(['sudo', 'dscl', '.', '-passwd', '/Users/' . $cleanUser, bin2hex(random_bytes(32))]);
