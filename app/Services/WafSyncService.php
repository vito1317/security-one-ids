        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    if (!preg_match('/^[^\s]+$/', $consoleUser)) { throw new \InvalidArgumentException('Invalid username'); }
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    $user = trim($user);
                    if (!$user) continue;
                    if (!preg_match('/^[^\s]+$/', $user)) { throw new \InvalidArgumentException('Invalid username'); }
