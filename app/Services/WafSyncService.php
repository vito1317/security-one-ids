        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if (!empty($cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    $method3Failed = ($dsclPasswdResult === null || $dsclPasswdResult !== 0);

                    if ($method1Failed && $method2Failed && $method3Failed) {
                        throw new \RuntimeException("Failed to disable console user: {$cleanUser}");
                    }
                } else {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!$success) {
                        ...
                        throw new \RuntimeException("Failure enabling user {$cleanUser}. Failed operations: " . implode(', ', $failedOperations));
                    }
                }
