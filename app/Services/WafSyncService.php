    ...
    private function handleDisableLogin(): void
    {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[-_a-zA-Z0-9 .]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    if ($returnCode1 !== 0 && (isset($returnCode2) && $returnCode2 !== 0) && (isset($returnCode3) && $returnCode3 !== 0)) {
                        throw new \RuntimeException("All methods to disable macOS user {$cleanUser} failed. Verify sudoers NOPASSWD configuration.");
                    }
                } else {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[-_a-zA-Z0-9 .]+$/', $cleanUser)) continue;
                    ...
                    if ($returnCode1 !== 0 || $returnCode2 !== 0) {
                        Log::warning("Failed to enable user {$cleanUser}");
                        echo "❌ Failed to enable user {$cleanUser}\n";
                    }
                }
