            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && $cleanUser !== 'root' && $cleanUser !== '_mbsetupuser' && preg_match('/^[a-zA-Z0-9._\- ]+$/', $cleanUser)) {
                    ...
                    try {
                        ...
                        $returnCode = $process->isSuccessful() ? 0 : ($process->exitCode() ?? 1);
                    ...
                    if ($returnCode !== 0) {
                        // Method 3: Set the account to expire immediately
                        try {
                            $process = Process::timeout(60)->run(['sudo', 'chpass', '-e', '0', $cleanUser]);
                            $returnCode = $process->isSuccessful() ? 0 : ($process->exitCode() ?? 1);
                            file_put_contents($logFile, "[{$timestamp}] chpass lock account: code={$returnCode}\n", FILE_APPEND);
                        } catch (\Exception $e) {
                            $returnCode = 1;
                        }
                    }
