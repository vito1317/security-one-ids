    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                // Sanitize user for logging to prevent log injection
                $logConsoleUser = preg_replace('/[\r\n]+/', ' ', $consoleUser);
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    if (!preg_match('/^[a-zA-Z0-9._-]+$/', $consoleUser)) {
                        file_put_contents($logFile, "[{$timestamp}] Invalid console user format: {$logConsoleUser}\n", FILE_APPEND);
                    } else {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9._-]+$/', $user)) {
                        ...
                        file_put_contents($logFile, "[{$timestamp}] Invalid user format skipped: {$logUser}\n", FILE_APPEND);
                        continue;
                    }
