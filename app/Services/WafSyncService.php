                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    if (!preg_match('/^[a-zA-Z0-9_.-]+$/', $consoleUser)) {
                        ...
                        if ($returnCode !== 0) {
                            ...
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$consoleUser}: code={$returnCode}\n", FILE_APPEND);
                        }

                        if ($returnCode !== 0) {
                            // Method 3: Set an impossible password hash
                            exec("sudo dscl . -passwd /Users/{$escapedUser} '*' 2>&1", $output, $returnCode);
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password {$consoleUser}: code={$returnCode}\n", FILE_APPEND);
                        }
                    }
                } else {
