        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    } catch (\Illuminate\Process\Exceptions\ProcessTimedOutException|\Illuminate\Process\Exceptions\ProcessFailedException $e) {
                        $method1Success = false;
                        $returnCode1 = 1;
                    ...
                    if (!$method1Success) {
                        ...
                        if (!$method2Success) {
                            ...
                            try {
                                ...
                                $returnCode3 = 1;
                                file_put_contents($logFile, "[{$timestamp}] dscl set impossible password exception: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                            } catch (\Exception $e) {
                                $returnCode3 = 1;
                                file_put_contents($logFile, "[{$timestamp}] dscl set impossible password exception: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                            }
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    try {
                        ...
                        file_put_contents($logFile, "[{$timestamp}] error enabling user {$cleanUser}: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                    } catch (\Exception $e) {
                        file_put_contents($logFile, "[{$timestamp}] error enabling user {$cleanUser}: " . get_class($e) . ': ' . $e->getMessage() . "\n", FILE_APPEND);
                    }
                }
