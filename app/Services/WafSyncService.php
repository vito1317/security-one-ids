    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                $cleanUser = trim($user);
                file_put_contents($logFile, "[{$timestamp}] Raw console user: {$user}, Sanitized: {$cleanUser}\n", FILE_APPEND);
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9_][a-zA-Z0-9._-]*$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    try {
                        $process1 = Process::timeout(60)->run("sudo dscl . -create /Users/{$safeUser} AuthenticationAuthority ';DisabledUser;'");
                        ...
                        $method1Success = false;
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} timeout: " . $e->getMessage() . "\n", FILE_APPEND);
                        throw $e;
                    } catch (\Exception $e) {
                        $method1Success = false;
                        $returnCode1 = 1;
                    ...
                    if (!$method1Success) {
                        ...
                        try {
                            $process2 = Process::timeout(60)->run("sudo pwpolicy -u {$safeUser} disableuser");
                            ...
                            $method2Success = false;
                            $returnCode2 = 1;
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user {$cleanUser} timeout: " . $e->getMessage() . "\n", FILE_APPEND);
                            throw $e;
                        } catch (\Exception $e) {
                            $method2Success = false;
                            $returnCode2 = 1;
                        ...
                        if (!$method2Success) {
                            ...
                            try {
                                $process3 = Process::timeout(60)->run("sudo chpass -e 0 {$safeUser}");
                                ...
                                $method3Success = false;
                                $returnCode3 = 1;
                                file_put_contents($logFile, "[{$timestamp}] chpass set expire immediately timeout: " . $e->getMessage() . "\n", FILE_APPEND);
                                throw $e;
                            } catch (\Exception $e) {
                                $method3Success = false;
                                $returnCode3 = 1;
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    $returnCode1 = null;
                    try {
                        $process1 = Process::timeout(60)->run("sudo -n dscl . -delete /Users/{$safeUser} AuthenticationAuthority");
                        $method1Success = $process1->successful();
                        $returnCode1 = $process1->exitCode();
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser}: code={$returnCode1}\n", FILE_APPEND);
                    } catch (\Illuminate\Process\Exceptions\ProcessTimedOutException $e) {
                        $returnCode1 = 1;
                        file_put_contents($logFile, "[{$timestamp}] dscl clear auth for {$cleanUser} timeout: " . $e->getMessage() . "\n", FILE_APPEND);
                        continue;
                    } catch (\Exception $e) {
                        $returnCode1 = 1;
                    ...
                    $returnCode2 = null;
                    try {
                        $process2 = Process::timeout(60)->run("sudo -n pwpolicy -u {$safeUser} enableuser");
                        $method2Success = $process2->successful();
                        $returnCode2 = $process2->exitCode();
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser}: code={$returnCode2}\n", FILE_APPEND);
                    } catch (\Illuminate\Process\Exceptions\ProcessTimedOutException $e) {
                        $returnCode2 = 1;
                        file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user {$cleanUser} timeout: " . $e->getMessage() . "\n", FILE_APPEND);
                        continue;
                    } catch (\Exception $e) {
                        $returnCode2 = 1;
