    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                Log::debug('WAF Sync: Processing macOS user for disable', ['cleanUser' => $cleanUser]);
                ...
                if ($cleanUser && $cleanUser !== 'root' && $cleanUser !== '_mbsetupuser' && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser)) {
                    ...
                    try {
                        ...
                        Log::error('Process execution failed for Method 1', ['exception' => $e->getMessage()]);
                        $returnCode = 1;
                    }
                    ...
                    if (!$methodSucceeded) {
                        try {
                            ...
                            Log::error('Process execution failed for Method 2', ['exception' => $e->getMessage()]);
                            $returnCode = 1;
                        }
                    }
                    ...
                    if (!$methodSucceeded) {
                        try {
                            ...
                            Log::error('Process execution failed for Method 3', ['exception' => $e->getMessage()]);
                        }
                    }
                } else {
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    Log::debug('WAF Sync: Processing macOS user for enable', ['cleanUser' => $cleanUser]);

                    if (strpos($cleanUser, ' ') !== false || !preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser)) {
                        continue;
                    }
