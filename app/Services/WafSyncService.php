        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    $method1Failed = !$userDisabledSuccessfully;
                    if ($method1Failed) {
                        ...
                        $method2Failed = !$userDisabledSuccessfully;
                        if ($method2Failed) {
                    ...
                    if (!$userDisabledSuccessfully) {
                        Log::error("Critical failure: Could not disable user {$cleanUser} via dscl, pwpolicy, or dscl passwd.");
                        throw new \Exception("Critical failure: Could not disable user {$cleanUser}");
                    }
                } else {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!$success) {
                        ...
                        throw new \Exception("Critical failure: Could not enable user {$cleanUser}");
                    }
                }
