    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    if (!$method1Success) {
                        ...
                        if (!$method2Success) {
                            ...
                            if (!$method3Success) {
                                throw new \Exception("Failed to disable login for user {$cleanUser} (all methods failed).");
                            }
                        }
                    }
                } else {
                ...
                exec("awk -F: '$3 >= 1000 && $3 < 65534 {print $1}' /etc/passwd 2>/dev/null", $users, $returnCode);

                $failCount = 0;
                $processedCount = 0;

                if ($returnCode === 0 && !empty($users)) {
                    foreach ($users as $user) {
                        ...
                        $processedCount++;
                        try {
                            $process = Process::timeout(60)->run(['passwd', '-l', $user]);
                            if (!$process->isSuccessful()) {
                                $failCount++;
                                file_put_contents($logFile, "[{$timestamp}] Failed to disable Linux user {$user}: code={$process->exitCode()}\n", FILE_APPEND);
                            }
                        } catch (\Exception $e) {
                            $failCount++;
                            file_put_contents($logFile, "[{$timestamp}] Failed to disable Linux user {$user} exception: " . $e->getMessage() . "\n", FILE_APPEND);
                        }
                    }
                }

                if ($processedCount > 0 && $failCount === $processedCount) {
                    throw new \Exception("Failed to disable login for all {$processedCount} Linux users.");
                } elseif ($failCount > 0) {
                    Log::warning("Failed to disable login for {$failCount} out of {$processedCount} Linux users.");
                }

                file_put_contents($logFile, "[{$timestamp}] Linux disable users result: processed={$processedCount}, failed={$failCount}\n", FILE_APPEND);
            }
