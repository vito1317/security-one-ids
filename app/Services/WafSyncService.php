            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    try {
                        ...
                        $rawOutput = trim($process1->getOutput() . ' ' . $process1->getErrorOutput());
                        $cleanOutput = substr(preg_replace('/[^a-zA-Z0-9\s.,_-]/', '', $rawOutput), 0, 255);
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser}: code={$dsclDisableResult}, output={$cleanOutput}\n", FILE_APPEND);
                    } catch (\Exception $e) {
