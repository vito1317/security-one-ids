            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($consoleUser && $consoleUser !== 'root' && $consoleUser !== '_mbsetupuser') {
                    if (!preg_match('/^[a-zA-Z0-9_.-]+$/', $consoleUser)) {
                        Log::error('Invalid console user characters: ' . $consoleUser);
                        file_put_contents($logFile, "[{$timestamp}] Invalid console user characters: {$consoleUser}\n", FILE_APPEND);
                    } else {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    if (!preg_match('/^[a-zA-Z0-9_.-]+$/', $user)) {
                        Log::error('Invalid user characters: ' . $user);
                        file_put_contents($logFile, "[{$timestamp}] Invalid user characters: {$user}\n", FILE_APPEND);
                        continue;
                    }
