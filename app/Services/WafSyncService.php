            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if ($cleanUser && preg_match('/^[a-zA-Z0-9._-]+$/', $cleanUser) && $cleanUser !== 'root' && $cleanUser !== 'daemon' && $cleanUser !== 'nobody' && $cleanUser !== '_mbsetupuser') {
                    ...
                    try {
                        ...
                        $dsclDisableResult = $process1->isSuccessful() ? 0 : ($process1->getExitCode() ?? 1);
                    ...
                    if ((!$dsclDisableExecuted || $dsclDisableResult !== 0)) {
                        ...
                        try {
                            ...
                            $pwpolicyDisableResult = $process2->isSuccessful() ? 0 : ($process2->getExitCode() ?? 1);
                    ...
                    if (!$disableSuccess) {
                        ...
                        try {
                            ...
                            $process3->run();
                            $dsclPasswdResult = $process3->isSuccessful() ? 0 : ($process3->getExitCode() ?? 1);
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    try {
                        ...
                        $dsclClearResult = $process1->isSuccessful() ? 0 : ($process1->getExitCode() ?? 1);
                    ...
                    if (!$success) {
                        ...
                        throw new \Exception("Failed to enable macOS user login for {$cleanUser}.");
                    }
                }
