    ...
    private function handleDisableLogin(): void
    {
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                file_put_contents($logFile, "[{$timestamp}] Console user: " . htmlspecialchars($cleanUser, ENT_QUOTES, 'UTF-8') . "\n", FILE_APPEND);
                ...
                if ($cleanUser && $cleanUser !== 'root' && $cleanUser !== '_mbsetupuser' && preg_match('/^[a-zA-Z0-9._\- ]+$/', $cleanUser)) {
                    ...
                    file_put_contents($logFile, "[{$timestamp}] dscl disable user " . htmlspecialchars($cleanUser, ENT_QUOTES, 'UTF-8') . ": code={$returnCode}, output=" . htmlspecialchars($outputStr, ENT_QUOTES, 'UTF-8') . "\n", FILE_APPEND);
        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                foreach ($usersOutput as $user) {
                    ...
                    $returnCode = $process->exitCode();
                    file_put_contents($logFile, "[{$timestamp}] dscl clear auth for " . htmlspecialchars($cleanUser, ENT_QUOTES, 'UTF-8') . ": code={$returnCode}\n", FILE_APPEND);
                    
                    // Re-enable with pwpolicy  
                    $process = Process::timeout(60)->run(['sudo', 'pwpolicy', '-u', $cleanUser, 'enableuser']);
                    $returnCode = $process->exitCode();
                    file_put_contents($logFile, "[{$timestamp}] pwpolicy enable user " . htmlspecialchars($cleanUser, ENT_QUOTES, 'UTF-8') . ": code={$returnCode}\n", FILE_APPEND);
                }
