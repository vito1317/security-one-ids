        ...
        try {
            ...
            if (PHP_OS_FAMILY === 'Windows') {
                ...
                if (!preg_match('/^[a-zA-Z0-9_][a-zA-Z0-9._-]*$/', $cleanUser)) {
                    file_put_contents($logFile, "[{$timestamp}] Invalid console user format: " . htmlspecialchars($cleanUser, ENT_QUOTES, 'UTF-8') . "\n", FILE_APPEND);
                    $cleanUser = null;
                }
                ...
                if ($cleanUser && $cleanUser !== 'root' && $cleanUser !== '_mbsetupuser') {
                    ...
                    exec("sudo -n dscl . -create /Users/{$escapedUser} AuthenticationAuthority ';DisabledUser;' 2>&1", $output, $returnCode);

                    if ($returnCode !== 0) {
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} FAILED: [operation failed]\n", FILE_APPEND);
                    } else {
                        file_put_contents($logFile, "[{$timestamp}] dscl disable user {$cleanUser} SUCCESS: code={$returnCode}\n", FILE_APPEND);
                    }
                    ...
                    if ($returnCode !== 0) {
                        ...
                        $output = [];
                        $returnCode = -1;
                        exec("sudo -n pwpolicy -u {$escapedUser} disableuser 2>&1", $output, $returnCode);
                        if ($returnCode !== 0) {
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user FAILED: [operation failed]\n", FILE_APPEND);
                        } else {
                            file_put_contents($logFile, "[{$timestamp}] pwpolicy disable user SUCCESS: code={$returnCode}\n", FILE_APPEND);
                        }
                    }
                    ...
                    if ($returnCode !== 0) {
                        ...
                        $output = [];
                        $returnCode = -1;
                        exec("sudo -n dscl . -passwd /Users/{$escapedUser} '*' 2>&1", $output, $returnCode);
                        if ($returnCode !== 0) {
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password FAILED: [operation failed]\n", FILE_APPEND);
                        } else {
                            file_put_contents($logFile, "[{$timestamp}] dscl set impossible password SUCCESS: code={$returnCode}\n", FILE_APPEND);
                        }
                    }
                } else {
