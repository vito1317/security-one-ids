...
class LogDiscoveryService
{
    ...
    public function getCustomPaths(): array
    {
        if (!cache()->has('ids.custom_log_paths')) {
            $hasLegacyKeys = cache()->has('ids_custom_log_paths') || cache()->has('ids::custom_log_paths');
            ...
            if ($hasLegacyKeys) {
                ...
                try {
                    ...
                    try {
                        if ($acquired) {
                            ...
                            // Lock contention: another process is migrating.
                            // Read legacy keys directly while waiting so we don't block.
                ...
                if ($lockFailed && !cache()->has('ids.custom_log_paths')) {
                    $legacyPaths = array_merge(
                        (array) cache()->get('ids_custom_log_paths', []),
                        (array) cache()->get('ids::custom_log_paths', [])
                    );
                ...
                // Initialize cache if neither new nor legacy keys exist
