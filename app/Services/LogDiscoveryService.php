...
class LogDiscoveryService
{
    ...
    public function addCustomPath(string $path): bool
    {
        ...
        // Validate decoded input for path traversal characters
        ...
        if (is_link($originalPath) || is_link($realPath)) {
            return false;
        }
        ...
        try {
            ...
            Log::warning("Error acquiring lock to add custom log path", [
                'path' => $path,
                'error' => $e->getMessage()
            ]);
            return false;
        ...
        if (!$lockAcquired) {
            Log::warning("Could not acquire lock to add custom log path after timeout", ['path' => $path]);
            return false;
        }
        ...
        try {
            ...
            $normalizedConfigPaths = array_map(fn($p) => strtolower((string)realpath($p)), $configPaths);
            $normalizedRealPath = strtolower($realPath);

            if (in_array($normalizedRealPath, $normalizedConfigPaths, true)) {
                return true;
            }

            $cachedPaths = $this->getCustomPaths();
            $normalizedCachedPaths = array_map(fn($p) => strtolower((string)realpath($p)), $cachedPaths);

            // If it's already in the cache, we're good.
            if (in_array($normalizedRealPath, $normalizedCachedPaths, true)) {
                return true;
            }
    ...
    private function isAllowedPath(string $realPath): bool
    {
        ...
        $realPathTrimmed = rtrim($realPath, DIRECTORY_SEPARATOR);
        foreach (self::$resolvedBaseDirs as $realDir) {
            if ($realPathTrimmed === $realDir || str_starts_with($realPathTrimmed, $realDir . DIRECTORY_SEPARATOR)) {
                return true;
            }
        }
    ...
    public function getCustomPaths(): array
    {
        ...
        if (!self::$migrated && !cache()->has('ids.custom_log_paths_migrated')) {
            $attemptsKey = 'ids.custom_log_paths_migration_attempts';
            ...
            if ($attempts < 3) {
                ...
                if ($lock->get()) {
                    try {
                        if (!cache()->has('ids.custom_log_paths_migrated')) {
                    ...
                    Log::warning('Could not acquire lock for custom log paths migration.');
                    $legacyPaths = cache()->get('ids_custom_log_paths', []);
                    $currentPaths = cache()->get('ids.custom_log_paths', []);

                    if (!is_array($legacyPaths)) { $legacyPaths = []; }
                ...
                // If we've tried too many times, just assume it's migrated to prevent locking
                self::$migrated = true;
            }
        } else {
            self::$migrated = true;
