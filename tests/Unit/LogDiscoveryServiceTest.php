...
class LogDiscoveryServiceTest extends TestCase
{
    ...
    public function test_add_custom_path_adds_path_and_caches_when_path_already_in_config_but_not_in_cache(): void
    {
        if (!is_writable(sys_get_temp_dir())) {
            $this->markTestSkipped('Temp directory is not writable');
        }
        ...
        try {
            // Clear cache explicitly before setting up the scenario to guarantee a clean state
            cache()->forget('ids.custom_log_paths');
            ...
            // Verify cache IS set because it was missing from persistent cache
            $this->assertEquals([$tempPath], cache()->get('ids.custom_log_paths'));
        } finally {
            // Clean up
            if (file_exists($tempPath)) {
