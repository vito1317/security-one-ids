...
class LogDiscoveryServiceTest extends TestCase
{
    ...
    public function test_add_custom_path_returns_true_without_caching_when_path_already_in_config(): void
    {
        ...
        // Pre-warm the cache by calling getCustomPaths to avoid migration side-effects during addCustomPath
        $this->service->getCustomPaths();

        // Spy on Cache to ensure we don't unnecessarily write to it
        \Illuminate\Support\Facades\Cache::spy();

        $result = $this->service->addCustomPath($tempPath);

        $this->assertTrue($result);
        \Illuminate\Support\Facades\Cache::shouldNotHaveReceived('forever');
    }
