...
class LogDiscoveryServiceTest extends TestCase
{
    ...
    public function test_get_custom_paths_migrates_legacy_data_to_new_key(): void
    {
        \Illuminate\Support\Facades\Cache::store()->flush();
        config(['ids.custom_log_paths' => []]);

        $legacyData = ['/var/log/nginx/access.log'];
        \Illuminate\Support\Facades\Cache::forever('ids_custom_log_paths', $legacyData);

        $paths = $this->service->getCustomPaths();

        $this->assertEquals($legacyData, $paths);
        $this->assertEquals($legacyData, \Illuminate\Support\Facades\Cache::get('ids.custom_log_paths'));
        $this->assertFalse(\Illuminate\Support\Facades\Cache::has('ids_custom_log_paths'));
    }
}
